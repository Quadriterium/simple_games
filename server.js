...
// --- Fallback routes for /geographie/* ---
const geographieSubPages = ['departements', 'continents'];
geographieSubPages.forEach(page => {
    app.get(`/geographie/${page}`, trailingSlashRedirect);
    app.get(`/geographie/${page}/`, (req, res) => {
        res.sendFile(path.join(__dirname, 'geographie', page, 'index.html'));
    });
});

app.get('/geographie', trailingSlashRedirect);
app.get('/geographie/', (req, res) => {
    res.sendFile(path.join(__dirname, 'geographie', 'index.html'));
});
